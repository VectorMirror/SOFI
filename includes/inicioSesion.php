<?php
    ...
    function loginUser($usuario, $password){
        global $DB_conection;
        $consulta = $DB_conection->prepare("SELECT usu_id, usu_rol, usu_pass FROM usuarios WHERE usu_correo = ? LIMIT 1");
        ...
        if($fila > 0){
            if(cuentaActiva ($usuario)){
                $consulta->bind_result($idUser, $idRol, $passDB);
                $consulta->fetch();
    ...
    function cuentaActiva($usuario){
        global $DB_conection;
        $consulta = $DB_conection->prepare("SELECT usu_activo FROM usuarios WHERE usu_id= ? LIMIT 1");
